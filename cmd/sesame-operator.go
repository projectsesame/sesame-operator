...
func main() {
	flag.StringVar(&opCfg.SesameImage, "sesame-image", config.DefaultSesameImage,
		"The container image used for the managed Sesame.")
	...
	images := []string{opCfg.SesameImage, opCfg.EnvoyImage}
	...
	setupLog.Info("using sesame", "image", opCfg.SesameImage)
	setupLog.Info("using envoy", "image", opCfg.EnvoyImage)

	op, err := operator.New(ctrl.GetConfigOrDie(), &opCfg)
	if err != nil {
		setupLog.Error(err, "failed to create sesame operator")
		os.Exit(1)
	}

	setupLog.Info("starting sesame operator")
	if err := op.Start(ctrl.SetupSignalHandler()); err != nil {
		setupLog.Error(err, "failed to start sesame operator")
		os.Exit(1)
	}
}

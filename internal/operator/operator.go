...
import (
	...
	"github.com/projectsesame/sesame-operator/internal/controller"
...
// Operator is the scaffolding for the sesame operator. It sets up dependencies
...
// +kubebuilder:rbac:groups=operator.projectsesame.io,resources=Sesames,verbs=get;list;watch;update
// +kubebuilder:rbac:groups=operator.projectsesame.io,resources=Sesames/status,verbs=get;update;patch
...
func New(cliCfg *rest.Config, opCfg *config.Config) (*Operator, error) {
	nonCached := []client.Object{&operatorv1alpha1.Sesame{}, &gatewayv1alpha2.GatewayClass{},
	...
	// Create and register the sesame controller with the operator manager.
	if _, err := controller.New(mgr, controller.Config{
		SesameImage: opCfg.SesameImage,
		EnvoyImage:  opCfg.EnvoyImage,
	}); err != nil {
		return nil, fmt.Errorf("failed to create sesame controller: %w", err)
	}

...
func JobConfigChanged(current, expected *batchv1.Job) (*batchv1.Job, bool) {
	...
	// labels and only check the presence of the sesame owning labels.
	if current.Spec.Template.Labels != nil {
		_, nameFound := current.Spec.Template.Labels[operatorv1alpha1.OwningSesameNameLabel]
		_, nsFound := current.Spec.Template.Labels[operatorv1alpha1.OwningSesameNsLabel]
...
// SesameStatusChanged checks if current and expected match and if not,
// returns true.
func SesameStatusChanged(current, expected operatorv1alpha1.SesameStatus) bool {
	if current.AvailableSesames != expected.AvailableSesames {
		return true
	}

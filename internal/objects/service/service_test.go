...
func TestDesiredSesameService(t *testing.T) {
	name := "svc-test"
...
func TestDesiredEnvoyService(t *testing.T) {
	...
	resourceGroup := "sesame-rg-test"
	subnet := "sesame-subnet-test"
	cfg := objSesame.Config{
		Name:        name,
		Namespace:   fmt.Sprintf("%s-ns", name),
		SpecNs:      "projectsesame",
		...
		NodePorts:   objSesame.MakeNodePorts(map[string]int{"http": 30081, "https": 30444}),
	}
	cntr := objSesame.New(cfg)

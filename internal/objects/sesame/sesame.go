...
// New makes a Sesame object using the provided ns/name for the object's
...
// OtherSesamesExist lists Sesame objects in all namespaces, returning the list
// and true if any exist other than sesame.
func OtherSesamesExist(ctx context.Context, cli client.Client, Sesame *operatorv1alpha1.Sesame) (bool, *operatorv1alpha1.SesameList, error) {
	Sesames := &operatorv1alpha1.SesameList{}
	if err := cli.List(ctx, Sesames); err != nil {
		return false, nil, fmt.Errorf("failed to list Sesames: %w", err)
	}
	if len(Sesames.Items) == 0 || len(Sesames.Items) == 1 && Sesames.Items[0].Name == Sesame.Name {
		return false, nil, nil
	}
	return true, Sesames, nil
}

// OtherSesamesExistInSpecNs lists Sesame objects in the same spec.namespace.name as sesame,
...
func OtherSesamesExistInSpecNs(ctx context.Context, cli client.Client, Sesame *operatorv1alpha1.Sesame) (bool, error) {
	exist, Sesames, err := OtherSesamesExist(ctx, cli, Sesame)
	if err != nil {
		return false, err
	}
	if exist {
		for _, c := range Sesames.Items {
			if c.Name == Sesame.Name && c.Namespace == Sesame.Namespace {
				// Skip the sesame from the list that matches the provided sesame.
				continue
			}
			if c.Spec.Namespace.Name == Sesame.Spec.Namespace.Name {
				return true, nil
			}
		}
	}
...
// OwningSelector returns a label selector using "sesame.operator.projectsesame.io/owning-sesame-name"
// and "sesame.operator.projectsesame.io/owning-sesame-namespace" labels.

...
func TestDesiredDeployment(t *testing.T) {
	...
	testSesameImage := config.DefaultSesameImage
	deploy := DesiredDeployment(cntr, testSesameImage)

	container := checkDeploymentHasContainer(t, deploy, SesameContainerName, true)
	checkContainerHasImage(t, container, testSesameImage)
	checkDeploymentHasEnvVar(t, deploy, SesameNsEnvVar)
	checkDeploymentHasEnvVar(t, deploy, SesamePodEnvVar)
...
func TestNodePlacementDeployment(t *testing.T) {
	name := "selector-test"
	selectors := map[string]string{"node-role": "sesame"}
	...
	cntr.Spec.NodePlacement = &operatorv1alpha1.NodePlacement{
		Sesame: &operatorv1alpha1.SesameNodePlacement{

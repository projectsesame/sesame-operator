...
func TestDesiredSesameConfigmap(t *testing.T) {
	expected := `#
# server:
...
`
	...
	cm, err := desired(configForSesame(c))
	require.NoError(t, err)
	require.Equal(t, "sesame", cm.Name)
	require.Equal(t, "some-ns", cm.Namespace)
	require.Contains(t, cm.Data, "sesame.yaml")
	assert.Equal(t, expected, cm.Data["sesame.yaml"])
}

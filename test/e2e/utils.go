...
import (
	...
	"github.com/projectsesame/sesame-operator/internal/parse"
...
func newSesame(ctx context.Context, cl client.Client, cfg objsesame.Config) (*operatorv1alpha1.Sesame, error) {
	cntr := objsesame.New(cfg)
	if err := cl.Create(ctx, cntr); err != nil {
		return nil, fmt.Errorf("failed to create sesame %s/%s: %v", cntr.Namespace, cntr.Name, err)
	}
	return cntr, nil
}

func updateSesame(ctx context.Context, cl client.Client, cfg objsesame.Config) (*operatorv1alpha1.Sesame, error) {
	desired := objsesame.New(cfg)
	cntr := &operatorv1alpha1.Sesame{}
...
func deleteSesame(ctx context.Context, cl client.Client, timeout time.Duration, name, ns string) error {
	cntr := &operatorv1alpha1.Sesame{
	...
	if err := cl.Delete(ctx, cntr); err != nil {
		if !errors.IsNotFound(err) {
			return fmt.Errorf("failed to delete sesame %s/%s: %v", cntr.Namespace, cntr.Name, err)
		}
	}
	...
	if err != nil {
		return fmt.Errorf("timed out waiting for sesame %s/%s to be deleted: %v", cntr.Namespace, cntr.Name, err)
	}
	return nil
}
...
func WaitForSesameAvailable(cl client.Client, name, ns string) error {
	...
	return wait.PollImmediate(1*time.Second, time.Minute*5, func() (bool, error) {
		...
		if cntr.Status.AvailableSesames != cntr.Spec.Replicas {
			return false, nil
		}
		...
		for _, c := range cntr.Status.Conditions {
			if c.Type == operatorv1alpha1.SesameAvailableConditionType && c.Status == metav1.ConditionTrue {
				return true, nil
			}
		}
...
// header "local.projectsesame.io" until timeout, expecting a 200 status code.
func podWaitForHTTPResponse(ctx context.Context, cl client.Client, ns, name, url string, timeout time.Duration) error {
	...
	host := fmt.Sprintf("Host: %s", "local.projectsesame.io")

...
import (
	...
	"github.com/projectsesame/sesame-operator/test/e2e"
...
func installOperatorFromExample(t *testing.T, c client.Client) func(*testing.T, client.Client) {
	...
	operatorResources := []struct {
		...
		{name: "sesame configuration crd", obj: new(apiextensions_v1.CustomResourceDefinition)},
		{name: "sesame deployment crd", obj: new(apiextensions_v1.CustomResourceDefinition)},
		{name: "sesame crd", obj: new(apiextensions_v1.CustomResourceDefinition)},
...
func testGatewayExample(t *testing.T, c client.Client) {
	...
	SesameInstance := &operatorv1alpha1.Sesame{}
	require.NoError(t, decoder.Decode(SesameInstance))
	SesameInstance.Spec.Namespace = operatorv1alpha1.NamespaceSpec{
		Name: testNS.Name,
	}
	require.NoError(t, c.Create(context.TODO(), SesameInstance))
	t.Logf("created sesame: %s/%s\n", SesameInstance.Namespace, SesameInstance.Name)
	defer func() {
		require.NoError(t, c.Delete(context.TODO(), SesameInstance))
		// Wait for sesame to be deleted.
		require.Eventually(t, func() bool {
			return c.Get(context.TODO(), client.ObjectKeyFromObject(SesameInstance), SesameInstance) != nil
		}, time.Minute*3, time.Second)
		t.Logf("deleted sesame %s/%s\n", SesameInstance.Namespace, SesameInstance.Name)
	}()

	require.NoError(t, e2e.WaitForSesameAvailable(c, SesameInstance.Name, SesameInstance.Namespace))
	t.Log("sesame available")
	...
	require.NoError(t, e2e.NewHTTPRoute(c, "kuard", testNS.Name, "kuard", "app", "kuard", "local.projectsesame.io", gw.Name, gw.Namespace, int32(80)))
	t.Log("deployed test app and HTTPRoute")

	assert.NoError(t, e2e.WaitForHTTPResponse("http://local.projectsesame.io/", time.Minute))
...
func testBasicIngressExample(t *testing.T, c client.Client) {
	...
	_, thisFile, _, ok := runtime.Caller(0)
	require.True(t, ok, "could not get path to this source file (test/e2e/example/example_test.go)")
	exampleYAMLPath := filepath.Join(filepath.Dir(thisFile), "..", "..", "..", "examples", "sesame", "sesame-nodeport.yaml")
	file, err := os.Open(exampleYAMLPath)
	require.NoError(t, err)
	defer file.Close()
	decoder := yaml.NewYAMLToJSONDecoder(file)

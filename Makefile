...
IMAGE := ${REGISTRY}/Sesame-operator
...
# Docker labels to be applied to the sesame-operator image. We don't transform
...
DOCKER_BUILD_LABELS = \
	...
	--label "org.opencontainers.image.url=https://github.com/projectsesame/sesame-operator/" \
	--label "org.opencontainers.image.documentation=https://github.com/projectsesame/sesame-operator/" \
	--label "org.opencontainers.image.source=https://github.com/projectsesame/sesame-operator/archive/${BUILD_VERSION}.tar.gz" \
	--label "org.opencontainers.image.version=${BUILD_VERSION}" \
	--label "org.opencontainers.image.revision=${BUILD_SHA}" \
	--label "org.opencontainers.image.vendor=Project Sesame" \
	--label "org.opencontainers.image.licenses=Apache-2.0" \
	--label "org.opencontainers.image.title=Sesame-operator" \
	--label "org.opencontainers.image.description=Deploy and manage Sesame using an operator."
...
manager: generate fmt vet
	go build -mod=readonly -o bin/Sesame-operator cmd/Sesame-operator.go

# Run against the configured Kubernetes cluster in ~/.kube/config
run: generate fmt vet manifests install
	go run ./cmd/Sesame-operator.go
...
test-example: ## Test the example Sesame.
.PHONY: test-example
test-example:
...
# Generate Sesame's rendered CRD manifest (i.e. HTTPProxy).
# Remove when https://github.com/projectsesame/sesame-operator/issues/42 is fixed.
.PHONY: generate-Sesame-crds
generate-Sesame-crds:
	@./hack/generate-Sesame-crds.sh $(NEW_VERSION)
...
manifests: generate-Sesame-crds generate-gateway-crds
	$(CONTROLLER_GEN) $(CRD_OPTIONS) rbac:roleName=Sesame-operator webhook paths="./..." output:crd:artifacts:config=config/crd/bases
...
multiarch-build-push: ## Build and push a multi-arch sesame-operator container image to the Docker registry
	docker buildx build \
...
container: ## Build the sesame-operator container image
container:
	docker build \
		--build-arg "BUILD_VERSION=$(BUILD_VERSION)" \
		--build-arg "BUILD_BRANCH=$(BUILD_BRANCH)" \
...
push: ## Push the sesame-operator container image to the Docker registry
push: container
